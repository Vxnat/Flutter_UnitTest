...
class _MyStepperAppState extends State<MyStepperApp> {
  ...
  Step step3() {
    ...
    TextEditingController detailedAddressController =
        TextEditingController(text: _detailAddressController.text);
    return Step(
  ...
  void getDistrictListAndWardList() {
    var provinceIdTemp = getIdFromProvinceName(province);
    districtListChoosen.clear();
    districtListChoosen += districtList
        .where((district) => district.provinceId == provinceIdTemp)
        .toList();
    districtListChoosen.removeWhere((element) => element.name == '');
    var districtIdTemp = getIdFromDistrictName(district);
    wardListChoosen.clear();
    wardListChoosen +=
        wardList.where((ward) => ward.districtId == districtIdTemp).toList();
    wardListChoosen.removeWhere((element) => element.name == '');
  }

  Future<void> loadDataLocal() async {
    final localData = await SharedPreferences.getInstance();
    setState(() {
      _nameController.text = localData.getString('nameLocal') ?? '';
      name = _nameController.text;
      _dateController.text = localData.getString('dateLocal') ?? '';
      date = _dateController.text;
      _phoneController.text = localData.getString('phoneLocal') ?? '';
      phone = _phoneController.text;
      gender = localData.getString('genderLocal') ?? '';
      _emailController.text = localData.getString('emailLocal') ?? '';
      email = _emailController.text;
      province = localData.getString('provincesLocal') ?? '';
      if (province != '') {
        provinceList.removeWhere((element) => element.name == '');
      }
      district = localData.getString('districtLocal') ?? '';
      ward = localData.getString('wardLocal') ?? '';
      _detailAddressController.text =
          localData.getString('detailAddressLocal') ?? '';
    });
  }
  ...
  Future<void> saveDataLocal() async {
    final localData = await SharedPreferences.getInstance();
    setState(() {
      localData.setString('nameLocal', name);
      localData.setString('dateLocal', date);
      localData.setString('phoneLocal', phone);
      localData.setString('genderLocal', gender);
      localData.setString('emailLocal', email);
      localData.setString('provincesLocal', province);
      localData.setString('districtLocal', district);
      localData.setString('wardLocal', ward);
      localData.setString('detailAddressLocal', _detailAddressController.text);
    });
  }
}
